...
Dbox "TahoeDbox" (tempStateArray) right, bottom , 50 , 30 title: "Tahoe Activity-Based Travel Demand Model" Toolbox NoKeyboard
    init do
        RunMacro("TCB Init")
        shared stateArray
    ...
    //Hidden button
    Button "" 50, 29.55, .01, .01 do
        MessageBox("Hi",)
    endItem

    //Quit: User will press this button to exit
    Button "Quit" 15, 27.6, 20, 1.5 cancel do
        RunMacro("CloseActions",path,season_idx,cr_user_iters,model_iters,scenario_list,scenario_idx,pathArray,vm_size)
        RunMacro("close everything")
        Return()
    Enditem 
...
Dbox "OpenDocumentationFile" (pathArray) 40, 7 title: "Open Documenation"
    ...
    Button "Open" 3,4,12,1.5 do
        if doc_idx = null then do
            ...
            LaunchProgram(pathArray[13] + docFileArray[doc_idx])
            return()
        end
    enditem
...
Macro "RunJavaModel" (pathArray,vmSize)
    ...
    RunProgram("cmd /c java" + javaClassPath + javaVMArguments + " com.pb.tahoe.util.TahoeModelComponentRunner TahoeModel",)
EndMacro
