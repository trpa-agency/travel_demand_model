...
+  All I-E and E-I trips between an external district and an internal zone will use the same gate.
...
There are six(6) major steps during this phase, including:
...
summarize_data_1 <- function(sum_attribute){
    ...
    write_csv(data_temp, paste0("outputs/",sum_attribute,"_summary_1.csv"))
}
...
summarize_data_2 <- function(attribute_1, attribute_2){
    ...
    write_csv(data_temp, paste0("outputs/",attribute_1, "_", attribute_2, "_summary_2.csv"))
}
...
#Show the summaries
data_temp <- read_csv("outputs/short_distance_external_VMT_long_distance_external_VMT_summary_2.csv")
kable(data_temp, format.args = list(big.mark = ","), align = c(rep('c',ncol(data_temp))), caption = "VMT (Outside TRPA Region)") %>%
    kable_styling() 
data_temp <- read_csv("outputs/short_distance_internal_VMT_long_distance_internal_VMT_summary_2.csv")
kable(data_temp, format.args = list(big.mark = ","), align = c(rep('c',ncol(data_temp))), caption = "VMT (Within TRPA Region)") %>%
    kable_styling() 
data_temp <- read_csv("outputs/short_distance_trip_long_distance_trip_summary_2.csv")
kable(data_temp, format.args = list(big.mark = ","), align = c(rep('c',ncol(data_temp))), caption = "Total Number of Trips") %>%
    kable_styling() 
```

```{r,output=F,message=F,echo=F,eval=T,results=F}
...
## Validation with Additional Streetlight Data
Upon summarizing the number of trips by gates, the study team validated the statistics with a second Streetlight dataset, which was produced by running the "O-D Analysis with Middle Filters". By setting the middle filters at the 7 gate locations, the study team was able to obtain the number of I-E and E-I trips by gates during the same study periods. Observations are as follows:

+  Overall, the two datasets matched reasonably (3%).
+  Compared to the second Streetlight dataset, the original summary tends to overestimate the number of trips. This is because the original dataset captured all I-E and E-I trips, while the second dataset only reflected the trips using the 7 gates. 
+  In addition, some of the discrepancies can be attributed to the limitations of assuming that all I-E and E-I trips between an external district and an internal zone would use the same gate. Given the size of the external districts, it is possible that multiple gates are viable options for the same O-D pair. 
...
```{r,message=F, warning=F, eval=T}
# compute total trips with first dataset
trips_summary1 <- data_v6 %>% 
  mutate(trips_dataset1 = short_distance_trip + long_distance_trip) %>%
  select(gate, trips_dataset1) %>%
  rename("Gate" = "gate",
         "Total Trips (Dataset 1)" = trips_dataset1)

# compute total trips with the second dataset (O-D analysis with middle filter)
MF_gate_raw <- read.csv("../Data/All_trips_at_gate_locations/98753_ScreenlineAll_2020_01_24_mf_all.csv", stringsAsFactors = FALSE)
MF_gate_EE_raw <- read.csv("../Data/EE_trips_at_gate_locations/98845_EETrips_Analysis_2020_01_24_mf_all.csv", stringsAsFactors = FALSE)
internal = c("11","12","13","14","15","16","17","18")
MF_gate_processed <- MF_gate_raw %>%
  filter(substr(Day.Type,1,1) != "0") %>% 
  filter(substr(Day.Part,1,1) == "0") %>%
  rename(link_vol = Average.Daily.O.M.D.Traffic..StL.Volume.,
         link_name = Middle.Filter.Zone.Name) %>%
  filter(!(Origin.Zone.Name %in% internal & Destination.Zone.Name %in% internal)) %>%
  dplyr::select(link_vol, link_name) %>%
  group_by(link_name) %>%
  summarize(n = n(),
            SL_vol = sum(link_vol))
MF_gate_EE_processed <- MF_gate_EE_raw %>%
  filter(substr(Day.Type,1,1) != "0") %>% 
  filter(substr(Day.Part,1,1) == "0") %>%
  rename(link_vol = Average.Daily.O.M.D.Traffic..StL.Volume.,
         link_name = Middle.Filter.Zone.Name) %>%
...
trips_summary2 <- MF_gate_processed %>% select(-n) %>%
  left_join(MF_gate_EE_processed %>% select(-n), by = c("link_name" = "link_name"), suffix = c("_all","_EE")) %>%
  mutate(SL_vol_IE = SL_vol_all - SL_vol_EE,
         link_name= as.numeric(substr(link_name, 6 ,6))) %>%
  rename(gate = link_name) %>%
  select(gate, SL_vol_IE) %>%
  rename("Gate" = "gate",
         "Total Trips (Dataset 2)" = SL_vol_IE)

# combine the summary from 2 datasets 
trips_summary <- trips_summary1 %>%
  left_join(trips_summary2, by = c("Gate" = "Gate")) %>%
  adorn_totals("row") %>%
  mutate(diff = ((`Total Trips (Dataset 1)` - `Total Trips (Dataset 2)`)/`Total Trips (Dataset 2)` * 100) %>%round()) %>%
  rename("Difference (%)" = diff) %>%
  write_csv("outputs/trips_summary.csv")
```

```{r,message=F, warning=F, echo = F, result = "asis"}
#Show the summaries
data_temp <- read_csv("outputs/trips_summary.csv")
kable(data_temp, format.args = list(big.mark = ","), align = c(rep('c',ncol(data_temp))), caption = "Comparison of Two Streetlight Datasets") %>%
    kable_styling() 
```
